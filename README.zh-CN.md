...
这个 MCP 服务器通过 AI 驱动的转换支持**自然语言时间表达式**！AI 会自动将你的自然语言时间表达式转换为系统所需的标准日期格式。

### 工作原理

当你使用自然语言时间表达式如"明天下午3点"或"下周一"时，AI 会：
1. 理解你的自然语言输入
2. 将其转换为标准日期格式（YYYY-MM-DD 或 YYYY-MM-DD HH:mm:ss）
3. 在调用 MCP 工具时使用转换后的格式
