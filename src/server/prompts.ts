      ...
      createMessage(
        createStructuredPrompt({
          ...
          constraints: [
            ...
            '**Note formatting rules**: Apple Reminders does NOT render markdown. When creating or updating reminder notes:',
            ...
            'Add related reminders in notes using structured format with relationship types:',
            '  - Group reminders by relationship type: Dependencies, Follow-up tasks, Related reminders, Blocked by, Prerequisites',
            '  - Format: "Related reminders:\n[Dependencies/Follow-up tasks/etc]:\n- [Reminder Title] (ID: {reminderId}) (List Name)\n- [Another Title] (ID: {reminderId2})"',
            '  - Place related reminders section at the end of notes, after any CRITICAL information',
            '  - Example complete note format: "CRITICAL: Blocked by - Need approval from manager\n\nOriginal note content\n\nRelated reminders:\nDependencies:\n- [Get manager approval] (ID: abc123) (Work)"',
            'IMPORTANT: Use the format [Title] (ID: {reminderId}) to reference related reminders. The reminder ID is stored for reference but does not create a clickable link.',
            ...
            '  - For HIGH CONFIDENCE (>80%) time blocks: Actually call the calendar tool with action="create". Format: "HIGH CONFIDENCE (90%): Creating time block\nTool: calendar\nArgs: {action: "create", title: "Deep Work — Project X", startDate: "2025-11-04 14:00:00", endDate: "2025-11-04 16:00:00", targetCalendar: "Work", note: "Focused time for uninterrupted work on Project X"}"',
            '  - For MEDIUM CONFIDENCE (60-80%) time blocks: Provide recommendation in tool call format. Format: "MEDIUM CONFIDENCE (70%): RECOMMENDATION - Create time block\nSuggested tool call: calendar with {action: "create", title: "Deep Work — Project X", startDate: "2025-11-04 14:00:00", endDate: "2025-11-04 16:00:00", targetCalendar: "Work", note: "Focused time for uninterrupted work on Project X"}\nRationale: [brief explanation]"',
            ...
            '  - Name deep work blocks using the pattern "Deep Work — [Reminder Title]" so the calendar makes focus intent obvious and consistent with deep scheduling habits.',
            '  - Deep work blocks should last 90 minutes by default; extend to cover the full focus need but avoid going below 60 minutes unless the user specifies otherwise.',
            ...
            '  - Plan no more than two or three deep work blocks per day with 15-30 minute breaks between sessions to keep cognitive performance high.',
          ],
          outputFormat: [
          ...
          qualityBar: [
            ...
            'High-confidence actions (>80%) are ACTUALLY EXECUTED using MCP tool calls (reminders or calendar tools), not just described. Medium-confidence actions are provided as recommendations in tool call format.',
            'Each action includes specific tool calls (reminders or calendar) with exact property settings in proper MCP tool call format.',
            'Calendar tool is ONLY used when explicitly creating time blocks identified in the output. Regular tasks use reminders tool.',
            ...
            'Related reminders are referenced using structured formatting: [Title] (ID: {id}) grouped by relationship type (Dependencies, Follow-up tasks, etc.) and placed at the end of notes.',
            ...
            'Notes use plain text formatting only (no markdown checkboxes "- [ ]" or "- [x]").',
