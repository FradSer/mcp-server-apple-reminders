    ...
    messages: [
      createMessage(
        createStructuredPrompt({
          ...
          process: [
            'Analyze existing reminders: inventory by completion status, urgency signals (due dates, list assignments), and energy requirements.',
            'Categorize effort: Deep Work (60-90 minutes for cognitively demanding tasks) or Shallow Tasks (15-60 minutes for routine work). The system automatically reserves ~20% buffer time. Flag tasks >90 minutes for splitting.',
            'CREATE calendar time blocks immediately for tasks with duration ≥60 minutes and due date today. Name blocks "Deep Work — [Project]" and anchor to reminder due times. Search for duplicates by normalized title when creating reminders.',
          ...
          constraints: [
            ...
            'Keep all scheduling decisions within today unless user explicitly authorizes deferring beyond current day.',
            'Do not modify recurrence rules, attachments, or sub-tasks unless explicitly requested.',
            `Use exact format "${timeContext.currentDate} HH:mm:ss" (local time) for due dates marked as "today".`,
          ...
          outputFormat: [
            ...
            '### Deep work blocks — cognitively demanding tasks requiring focused attention (90-120 minutes recommended, minimum 60 minutes, max 4 hours total per day). Schedule during peak energy hours (9am-12pm). List with reminder title, list name, duration, and specific objective for each session.',
          ...
          qualityBar: [
            'Current state provides clear metrics about the task landscape.',
            'High-confidence actions (>80%) are EXECUTED immediately using tool calls. Medium-confidence (60-80%) provided as recommendations.',
            'Each action includes specific tool calls with exact properties and confidence level with rationale.',
            'Calendar tool used when creating time blocks for tasks with duration ≥60min and due date today.',
            'Time consistency enforced: immediate actions have urgent due dates, deep work scheduled during peak energy (9am-12pm).',
            'Deep work blocks are CREATED (not just described) for tasks with duration ≥60min or explicit focus markers. High-confidence (>80%) blocks EXECUTED via calendar.events tool calls.',
            'Deep work blocks are 90-120 minutes recommended (minimum 60 minutes), max 4 hours per day, with specific objectives.',
            'Shallow tasks are 15-60 minutes, scheduled during lower energy periods, batched when possible.',
            'Schedule includes implicit buffer time (~20%) as natural gaps—no explicit "Buffer Time" events created.',
            'Response focuses on actionable organization rather than extensive analysis.',
            'No duplicate reminders created; similar items merged or updated.',
            `Any due date labeled "today" uses ${timeContext.currentDate} in tool calls.`,
          ],
          calibration: [
            ...WORKLOAD_CALIBRATION,
            ...CONTEXT_CALIBRATION,
