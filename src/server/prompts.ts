      ...
      createMessage(
        createStructuredPrompt({
          ...
          process: [
            ...
            'Estimate effort and categorize each actionable reminder: Deep Work (60-90 minutes for cognitively demanding, focused work) or Shallow Tasks (15-60 minutes for all other work including quick wins, routine tasks, and administrative activities). The system will automatically reserve ~20% of the schedule as implicit buffer time for flexibility. Flag anything above 90 minutes for task splitting suggestions.',
            'Identify time block candidates: Proactively flag tasks that need calendar blocks based on: (a) duration estimates ≥60 minutes, (b) explicit focus/uninterrupted time markers in notes, (c) multiple related tasks with explicit due times today that can be batched into ≥60 minute blocks. For each candidate, assess confidence level and prepare to create blocks for high-confidence (>80%) cases.',
            ...
            'Before creating calendar.events blocks for due-today reminder clusters, call calendar.events with action="read" to inspect the relevant window and reuse an existing block when a matching event already covers the work.',
          ...
          constraints: [
            ...
            'Calendar time blocks MUST be deduplicated: always run calendar.events with action="read" for the prospective time range before creating a new block and skip creation if an equivalent event already exists.',
            ...
            'When effort exceeds 90 minutes, recommend splitting into multiple subtasks that each fit within Deep Work (60-90 min) or Shallow Tasks (15-60 min) bounds; highlight how related subtasks can share a deep work block.',
            'When calendar blocking is warranted, convert Deep Work tasks into 60-90 minute events and Shallow Tasks into 15-60 minute events, starting each block so it ends at or immediately before the reminder due timestamp.',
            'Leave natural gaps (15-30 minutes) between major blocks as implicit buffer time for transitions and unexpected work—do not create explicit "Buffer Time" calendar events.',
          ...
          outputFormat: [
            ...
            '### Deep work blocks — cognitively demanding tasks requiring focused attention (60-90 minutes each, max 4 hours total per day). Schedule during peak energy hours (9am-12pm). List with reminder title, list name, duration, and specific objective for each session.',
            '### Shallow tasks — all other work including quick wins, routine tasks, and administrative activities (15-60 minutes each). Schedule during lower energy periods, batch similar tasks when possible. The system automatically reserves ~20% of the schedule as gaps between blocks for flexibility.',
          ],
          qualityBar: [
            ...
            'Calendar time blocks reflect the read-before-create dedupe: the verification log mentions the calendar.events read check before any new block is created.',
            'Actions are clearly labeled with confidence levels (high >80%, medium 60-80%, low <60%) and include brief rationale.',
            'Time consistency is enforced: immediate actions have urgent due dates, shallow tasks have same-day due dates, deep work blocks are scheduled during peak energy hours.',
            'Due dates and list assignments align with action urgency: critical blockers = urgent due dates + important list, routine tasks = same-day due dates with appropriate timing.',
            'Deep work blocks are PROACTIVELY identified and created (not just described) when tasks have duration ≥60min, explicit focus markers, or form batchable clusters. High-confidence blocks (>80%) are EXECUTED via calendar.events tool calls; medium-confidence blocks are provided as recommendations with tool call format.',
            'Deep work blocks are genuinely cognitively demanding (60-90 minutes), scheduled during peak energy (9am-12pm), limited to max 4 hours per day, and include specific objectives.',
            'Shallow tasks encompass all non-deep-work activities (15-60 minutes), scheduled during lower energy periods, batched when possible for efficiency.',
            'Schedule includes implicit buffer time (~20% of working hours) as natural gaps between blocks—no explicit "Buffer Time" events are created.',
